  ...
  return (
    <div style={s.container}>
      ...
      <Palette
        ...
        onToggleCat={(cat) =>
          state.setOpenCats((prev) => ({ ...prev, [cat]: !prev[cat] }))
        }
        onDragStart={state.handleDragStart}
        onDoubleClick={(type) =>
          state.addNodeToCanvas(
            type,
            20 + state.nextDropPos.current * 10,
            20 + state.nextDropPos.current * 10,
          )
        }
      />

      {/* ── Center: topbar + sessions + canvas ─────────────────── */}
