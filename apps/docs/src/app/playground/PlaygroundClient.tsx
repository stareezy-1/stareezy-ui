...
export default function Demo() {
  return (
    ...
    button: {
      label: "Button",
      icon: "◈",
      code: `import { Button, HStack } from '@stareezy-ui/components'
  ...
  return (
    <VStack gap={8} style={{ maxWidth: 300 }}>
      {SWATCHES.map(({ label, value }) => (
        <Box key={label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: spacing[3].value, borderRadius: radius.md.value, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
...
function ThemeDemo() {
  ...
  return (
    <Box bg={isDark ? colors.raisinBlack[800] : colors.neutral[10]} p={spacing[4]} rounded={radius.lg} style={{ maxWidth: 300 }}>
      <Text type="M-heading-bold" text={isDark ? '🌙 Dark Mode' : '☀️ Light Mode'} color={isDark ? colors.neutral[10].value : colors.raisinBlack[800].value} />
      <Button variant="primary" text="Toggle Theme" onPress={toggleTheme} style={{ marginTop: spacing[3].value }} />
    </Box>
  )
}

export default function Demo() {
  return <ThemeProvider theme="light"><ThemeDemo /></ThemeProvider>
}`,
    },
  };
...
export default function PlaygroundClient() {
  ...
  return (
    <div className="playground-root">
      {/* ── Inner top bar (example tabs + controls) ───────────────────── */}
