/**
 * UploadFileModal — modal for file upload with placeholder and confirm/cancel.
 *
 * Exact port of IUploadFileModalProps from rekosistem-components.
...
// ---------------------------------------------------------------------------
// IUploadFileModalProps — exact faithful port
// ---------------------------------------------------------------------------
...
export interface IUploadFileModalProps {
  ...
  show: boolean;
  selected?: unknown; // DocumentPickerResult
  onClose(): void;
  onPressPositive: (val: unknown) => void;
  type?: string | string[];
  testID?: string;
  fileName?: string;
  isSupportPasteFromClipboard?: boolean;
  placeholder: string;
}
...
export const UploadFileModal: React.FC<IUploadFileModalProps> = ({
  ...
  const acceptType = Array.isArray(type) ? type.join(",") : type;
  const displayName = fileName ?? (selected ? "File selected" : "");
        ...
        >
          ...
          >
            <input
              ...
              aria-label={placeholder}
            />
            <span style={{ fontSize: spacing[32].value }}>📁</span>
  ...
  return (
    <Modal
      ...
      testID={testID}
      accessibilityLabel={title ?? "Upload file"}
    >
      <View
        style={{
          flex: 1,
      ...
      >
        ...
        >
          ...
          >
            <Text style={{ fontSize: spacing[32].value }} allowFontScaling={false}>📁</Text>
            <Text
              style={{ fontSize: spacing[14].value, color: themed.textSecondary, textAlign: "center" }}
              allowFontScaling={false}
            >
              {displayName || placeholder}
            </Text>
